        ...
        <v-container class="my-5">
            ...
            <v-row class="mb-3">
                <v-tooltip top>
                    <template v-slot:activator="{ on, attrs }">
                        <v-btn small text color="grey" @click="sortByProjectName()" v-bind="attrs" v-on="on">
                ...
                <v-btn small text color="grey" @click="sortByAssigner()">
                    <v-icon left small>mdi-account-check-outline</v-icon>
                    <span class="caption text-lowercase">By Assigner</span>
                </v-btn>
                <v-btn small text color="grey" @click="sortByDeadline()">
                    <v-icon left small>mdi-av-timer</v-icon>
                    <span class="caption text-lowercase">By Deadline</span>
                </v-btn>
                <v-btn small text color="grey" @click="sortByStatus()">
                    <v-icon left small>mdi-account-check-outline</v-icon>
            ...
            <v-card flat v-for="task in tasks" :key="task.title">
                <v-row :class="`rounded-0 pa-3 task ${task.status}`">
                    ...
                    <v-flex>
                        <div>
                            <v-chip outlined small :color="getColor(task.status)"
